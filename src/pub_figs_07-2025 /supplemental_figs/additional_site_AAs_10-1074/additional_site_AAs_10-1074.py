...
#In this file I am going to make the supplemental figure of allele frequency
#plots for the additional 10-1074 sites we mention in the manuscript.
...
par_list_1074 = ['1HB3', '1HD11K', '1HD10K', '1HD9K', '1HD7K', '1HD6K',
                 '1HD5K', '1HD4K', '1HD1', '1HC3', '1HC2']

#Here is the output folder
outFolder = 'additional_site_AAs_10-1074/'
outDir = '../../../../results/pub_figs_07-2025/supplemental_figs/'
site_inpath = '../../../../results/pub_figs_07-2025/supplemental_tables/variation_10-1074/10-1074_ancestral_loss_summary.csv'
...
# Get the shared sites in four or more participants
shared_sites = site_par_df[site_par_df['Participant count'] >= 4].copy()
shared_sites = shared_sites['HXB2 Coordinate'].unique()
...
for curr_site in shared_sites:
    ...
    print('------------')
    fig, ax = plt.subplots(2, int(np.ceil(len(site_participants) / 2)), sharey=True,
                figsize = (0.75 * len(site_participants)/2, 2.5))
    ax = ax.flatten()
    plot_freqs_df = []
    ...
    for index, curr_par in enumerate(site_participants):
        ...
        #only show the xticks for the second row
        if index < len(site_participants) / 2:
            curr_ax.set_xticklabels([])
        else:
            curr_ax.set_xticklabels(freqs_to_plot['time_int'].unique())
            curr_ax.set_xlabel('Trial week')
        
        #Put y axis labels on the first column
        if index % int(np.ceil(len(site_participants) / 2)) == 0:
            curr_ax.set_ylabel('Frequency')
        else:
            curr_ax.set_yticklabels([])
        
        #If there is no final axis, then remove it
        if index == len(site_participants) -1 and len(site_participants) % 2 == 1:
            no_final_ax = True
        else:
            no_final_ax = False
    ...
    plt.legend(handles = [mpl.patches.Patch(color = color_dict[name], label = name) for name in colors_seen_formatted],
                title = 'Amino acid', loc = 'lower left', bbox_to_anchor = (-3.75, -3), ncol = 7, columnspacing = 0.8,
                frameon = False, fontsize = 6, title_fontsize = 6)
    # plt.suptitle('Site ' + str(curr_site))
    # plt.tight_layout()
    print('Saving figure for site ' + str(curr_site))
    plt.suptitle('Site ' + str(curr_site), fontsize = 6, y = 1)
    plt.subplots_adjust(wspace=0.2, left = 0.05, right = 0.95, 
                        hspace= 1, top=0.9)
    
    plt.savefig(outDir + outFolder + 'site_' + str(curr_site) + '.png', dpi = 300, bbox_inches='tight')
    plt.close()